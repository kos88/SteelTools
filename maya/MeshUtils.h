 ...
 * @return a vector with the ordered indices
 */
    inline std::vector<int> createOrderedVertices(MObject& edgeObj, MObject& meshObj)
    {
        ...
        originalIndexes.reserve(adj.size());
     ...
     */
    inline std::pair<std::vector<MVector>, std::vector<int>> createOrderedVerticesAndPos(MObject& edgeObj, MObject& meshObj)
    {
        std::unordered_map<int, Neighbours> adj;
        const MFnMesh fnMesh(meshObj);
